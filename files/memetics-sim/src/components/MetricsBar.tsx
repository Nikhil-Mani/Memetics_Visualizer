...
import { num, pct, rampCss, METRIC_COLORS } from './palette';
  ...
  return (
    <div className="bg-hair">
      <div className="flex flex-wrap items-baseline gap-x-5 gap-y-1 border-b border-hair bg-panel px-5 py-3" data-testid="mutation-summary">
        <span className="text-sm text-mist">Mutated this run <strong className="font-mono text-cyan" style={{ color: METRIC_COLORS.mutation }} data-testid="mutated-share">{pct(metrics.mutatedShare, 1)}</strong></span>
        ...
        <span className="text-xs text-slate">Highest generation: <strong style={{ color: METRIC_COLORS.mutation }} data-testid="max-generation">{metrics.maxGeneration}</strong></span>
      ...
      <Panel
        ...
        accent={METRIC_COLORS.polarization}
      >
        <Sparkline values={bimodal.values} color={METRIC_COLORS.polarization} min={0.1} max={1} baseline={5 / 9} />
      </Panel>
      ...
      <Panel
        ...
        accent={METRIC_COLORS.transmission}
      >
