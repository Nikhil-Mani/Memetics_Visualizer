...
Restart `npm run dev` after changing environment variables. `POST /api/interpret` is served by Vite's backend middleware in development and by `api/interpret.ts` as a Vercel Node function in production. Import this repository into Vercel with the project root set to this directory; `vercel.json` builds `dist` and configures the function runtime. Add `SEMANTIC_BASE_URL`, `SEMANTIC_MODEL`, and `SEMANTIC_API_KEY` as server-side Vercel environment variables. Credentials remain server-side and `.env.local` is gitignored. Only the retrieved evidence is sent to the configured model provider; this happens on explicit interpretation actions, not on each mutation. Requests have payload/shape validation, a 60-second upstream timeout, and one simultaneous interpretation per server.
